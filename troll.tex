...
\begin{frame}\frametitle{Oritatami systems at delay 1}
  \begin{block}{The two ways to stabilize a bead at delay 1}
    \begin{itemize}
...
%		\draw[thick, dotted] (0:0) -- (120:1);
	...
	\begin{tikzpicture}
		...
		\only<2>{
		\node[shift=(180:1)] at (-90:2) {free hands: $\pm 0$};
		}
...
\begin{block}{Tunnel Troll Theorem}
\begin{description}
	\item[$\alpha \geq 4$] \# of free hands does not increase when using a tunnel section.
\end{description}
\end{block}
\begin{center}
\begin{columns}[c]
...
\begin{block}{Tunnel Troll Theorem}
\begin{description}
	\item[$\alpha = 3$] At least one free hand is consumed / tunnel section.
\end{description}
\end{block}
	...
	\begin{tikzpicture}
		\draw[white] (60:1) -- (0:0);
			...
			\draw[olive, shift=(0:1.5)] (0:1.5) circle [radius=0.1];
			 ...
			 \node at (0,-2) {terminal conformation};
	          }
	\end{tikzpicture}
	\end{column}
...
\begin{block}{Tunnel Troll Theorem}
\begin{description}
	\item[$\alpha = 2$] At least one free hand is consumed / tunnel.
\end{description}
\end{block}
...
\begin{frame}\frametitle{Deterministic unary oritatami systems at delay 1}
  \begin{block}{Jordan curve theorem}
    A closed curve which is non-self-intersecting, divides the plane into inside and outside.
  \end{block}
...
At $\alpha = 2$, Troll consumes free hands at an entrance of tunnel, too.
